...
/datum/outfit/job/roguetown/post_equip(mob/living/carbon/human/H, visualsOnly = FALSE)
	...
	if(H.mind)
		...
		// Ensure Wretches and Assassins are granted their antagonist datum at post-equip
		...
		if(H.mind.assigned_role == "Assassin" && !H.mind.has_antag_datum(/datum/antagonist/assassin))
			H.mind.add_antag_datum(/datum/antagonist/assassin)
