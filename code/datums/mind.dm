...
/datum/mind/proc/recall_targets(mob/recipient, window=1)
	...
	for(var/mob/living/carbon in GLOB.mob_living_list) // Iterate through all mobs in the world
		if((carbon.real_name != recipient.real_name) && ((carbon.has_flaw(/datum/charflaw/hunted)) && (!istype(carbon, /mob/living/carbon/human/dummy)) && (!isdead(carbon))))//To be on the list they must be hunted, not be the user, not be a dummy (There is a dummy that has all vices for some reason) and be alive
